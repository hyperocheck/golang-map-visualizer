		...
		if liveMode {
			...
			for inserted < count {
				...
				if BUCKET_NUM == CheckHash(m, key) {
					...
					inserted++
					ctx.PrintfLogEvent("[%d/%d] Inserted key=%v, attempts=%d, time=%v\n", inserted, count, probe, totalAttempts, elapsed)
					ws.NotifyUpdate()
					totalAttempts = 0
			...
			ws.NotifyUpdate()
			ctx.PrintfLogEvent("Inserted %d keys, total attempts: %d\n", count, totalAttempts)
		}
		totalAttempts = 0
	...
	m.Console.RegisterCommand("range", "range <from> <to> [--life] — insert range of keys (map[int]int only)", func(ctx *ishell.Context) {
		...
		if liveMode {
			...
			for i := from; i <= to; i++ {
				...
				ctx.PrintfLogEvent("Insert key %v : val %v\n", key, val)
			}
		} else {
			ctx.PrintfLog("Range mode (batch): inserting keys from %d to %d...\n", from, to)
			for i := from; i <= to; i++ {
				key := any(i).(K)
				val := any(i).(V)
		...
		ctx.PrintfLogEvent("Range completed! Inserted: %d new keys, Updated: %d existing keys\n", inserted, updated)
	})
}
...
func (m *Meta[K, V]) registerShow() {
	m.Console.RegisterCommand("show", "show — print all key-value pairs", func(ctx *ishell.Context) {
		...
		if mapSize > 100 {
			ctx.PrintfLogWarn("Map contains more than 100 elements. Are you sure? (y/n): ")
			answer := ctx.ReadLine()
		...
		for k, v := range m.Map {
			ctx.PrintfLogEvent("%v : %v\n", k, v)
		}
		ctx.PrintfLogGood("Total: %d pairs\n", mapSize)
	})
}
