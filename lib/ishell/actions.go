...
type Actions interface {
	...
	// PrintlnLog prints to output without shift
	PrintlnLog(val ...interface{})
...
func (s *shellActionsImpl) PrintlnLog(val ...interface{}) {
	s.reader.scanner.Clean()

	//fmt.Fprint(s.writer, "\033[1A")

	fmt.Fprintln(s.writer, val...)
	s.reader.scanner.Refresh()
}
